...
## Style
Starting with this homework, **we will be enforcing style**. You must follow the [style guide](../../../resources/guides/style/index.md), or you will be penalized on the autograder.

You can and should check your style locally with the CS 61B plugin.
...
- `takeOrder`: This method takes in a `String` and returns a _new_ array containing the orders of the customer. If the customer is `Ergun`, you should return an array of Strings `["beyti", "pizza", "hamburger", "tea"]` in that order. If the customer is `Erik`, you should return an array of Strings `["sushi", "pasta", "avocado", "coffee"]`. In any other case, return a String array of size 3 where each element is `null`.
  {: .info}
  > **NOTE:** `==` behaves strangely with `String`s for reasons we'll see later in the course. You should check strings `s1` and `s2` for equality using `s1.equals(s2)` in Java. Also, `null` is the default value of elements in an initialized `String[]`.
- `findMinMax`: This method takes an `int[] array` and returns the positive difference between the maximum element and minimum element of the given array. You may assume the input array is nonempty.
